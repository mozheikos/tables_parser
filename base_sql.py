def get_sql(argument: str) -> str:
    sql = f"""
                ...
                from (
                        ...
                        (
                            select distinct
                            c.value as wallet, 'user' as type, u.login as name
                            from 
                        ...
                        (
                            select
                            wallet, 'other' as type,
                            IF(not trim(forum_login) = '', forum_login,
                            IF(not trim(forum_name)='', forum_name,
                            IF(not trim(tg_username) = '', CONCAT('tg: ', tg_username), CONCAT('jury ', subgov)))) as name
                            from
